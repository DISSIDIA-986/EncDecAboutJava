 ...
 *
 */
public abstract class CertificateCoderP12 {
	...
	private static KeyStore getKeyStore(String keyStorePath, String password)throws Exception {
		//实例化密钥库
		KeyStore ks = KeyStore.getInstance(KEYSTORE_TYPE_P12);
		//获得密钥库文件流
	...
	public static byte[] decryptByPrivateKey(byte[] data,String keyStorePath,String alias,String password)throws Exception {
		...
		/**
		 * 因为batchContent内容过长，为了避免 javax.crypto.IllegalBlockSizeException: Data must not be longer than 117 bytes 异常
		 * 拆分为多次加密
		 * 128
		 */
		for (int i = 0; i < data.length; i += 128) {
            byte[] subarray = ArrayUtils.subarray(data, i,i + 128);
            byte[] result = cipher.doFinal(subarray);
            mergedBytes = ArrayUtils.addAll(mergedBytes, result);
        }
		return mergedBytes;
	}
	/**
	...
	public static byte[] encryptByPublicKey(byte[] data,String certificatePath)throws Exception {
		...
		for (int i = 0; i < data.length; i += 100) {
            byte[] subarray = ArrayUtils.subarray(data, i,i + 100);
            byte[] result = cipher.doFinal(subarray);
            mergedBytes = ArrayUtils.addAll(mergedBytes, result);
        }
		return mergedBytes;
